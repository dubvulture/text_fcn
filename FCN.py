...
def main(argv=None):
    ...
    zeros = tf.reduce_sum(tf.cast(tf.equal(squeezed, 0), tf.float32))
    ones = tf.reduce_sum(tf.cast(tf.equal(squeezed, 1), tf.float32))
    weights = tf.one_hot(squeezed, 1, on_value=1., off_value=(ones/(zeros+ones)), dtype=tf.float32)
    ...
    opt = {'batch': FLAGS.batch_size, 'smin': 96, 'smax': 512, 'size': IMAGE_SIZE}
    ...
    if FLAGS.mode == "train":
        ...
        for itr in xrange(MAX_ITERATION):
            train_images, train_annotations, _ = train_dataset_reader.next_batch()
        ...
        valid_images, valid_annotations, valid_filenames = validation_dataset_reader.get_random_batch(FLAGS.batch_size)
        ...
        for itr in range(FLAGS.batch_size):
            utils.save_image(
                    ...
                    name="inp_" + str(5+itr))
            utils.save_image(
                    coco_utils.to_mask(valid_annotations[itr]),
                    FLAGS.logs_dir,
                    name="gt_" + str(5+itr))
            utils.save_image(
